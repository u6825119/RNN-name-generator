...
def seqs_to_ids(seqs, char_to_id, max_len=20):
    ...
    for name in seqs:
        narray = np.zeros(max_len)
        for i in range(0, len(name)):
            if i < max_len:
                key = name[i]
                narray[i] = char_to_id[key]
        narray = narray.astype(int)    
        all_seqs.append(narray)    
...
class RNNLM(nn.Module):
    ...
    def forward(self, x, h_last=None):
        ...
        out, h = self.gru(embeddings, h_last)
        out = self.linear(out)
...
def train_model(model, Xtrain, Ytrain, Xval, Yval, id_to_char, max_epoch):
    ...
    for e in range(max_epoch):
        print('Epoch: {}/{}.............'.format(e+1, max_epoch), end=' ')
        model.zero_grad()
        total_loss = 0
        ...
        print("loss: {:.4f}".format(avg_loss), " - val_loss: {:.4f}".format(val_loss))
...
def gen_string(model, id_to_char, max_len=20, sample=True):
    ...
    for i in range(max_len):
        ...
        if sample == True: #sampling
            con_prob = x.data.view(-1).div(0.3).exp() ## Softmax + temperature sampling from prob distribution
            probas = torch.multinomial(con_prob, 1)[0] 
            x = probas
        else:    # argmax
            x = torch.argmax(x)
            
        xid = x.item()
        if xid == 2: #end of name (id of '.')
            break
        out_char = id_to_char[xid]
        x = torch.tensor([[xid]]) # feed as next input
        out_str += out_char       # add char to generated string
...
def main():
    ...
    print("Random:")
    for i in range(10):
        gstr = gen_string(model, id_to_char, sample=True)
        print(gstr)
